...
function mgf( t, options ) {
	...
	if ( isNumber( t ) ) {
		return mgf1( t, opts.k, opts.lambda );
	}
	if ( isMatrixLike( t ) ) {
		if ( opts.copy !== false ) {
		...
		return mgf5( out, t, opts.k, opts.lambda );
	}
	if ( isTypedArrayLike( t ) ) {
		...
		return mgf6( out, t, opts.k, opts.lambda );
	}
	if ( isArrayLike( t ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return mgf4( t, opts.k, opts.lambda, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return mgf3( out, t, opts.k, opts.lambda, opts.accessor );
		}
		return mgf2( out, t, opts.k, opts.lambda );
	}
	return NaN;
} // end FUNCTION mgf()
